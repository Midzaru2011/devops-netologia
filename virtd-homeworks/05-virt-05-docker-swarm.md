...
При запуске режима `replication`, мы можем запустить желаемое колличество реплик сервиса, при этом, распределение реплие будет происходить рандомно, тоесть контенеры могут быть все запущены на одной ноде, или чразбиты по разным, но так чтобы сохранилось колличество обозначенных реплик. 
